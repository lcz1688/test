package lc.cn.thirdplatform.sharesdk.login;
...
public class LoginApi implements Callback {
		...
		plat.setPlatformActionListener(new PlatformActionListener() {
			public void onComplete(Platform plat, int action, HashMap<String, Object> res) {
				if (action == Platform.ACTION_USER_INFOR) {
					...
					msg.arg2 = action;
					res.put("uid", plat.getDb().getUserId());
					LogUtil.log(plat.getName()+"===uid===" + plat.getDb().getUserId());
	...
	public boolean handleMessage(Message msg) {
		...
		case MSG_AUTH_COMPLETE: {
			...
			LogUtil.log(objs.toString() + "----------" + plat);
			@SuppressWarnings("unchecked")
			HashMap<String, Object> res = (HashMap<String, Object>) objs[1];
			LogUtil.log(res.toString() + "----------" + plat);
			if (loginListener != null && loginListener.onLogin(plat, res)) {
				/*
				 * RegisterPage.setOnLoginListener(loginListener);
				 * RegisterPage.setPlatform(plat); Intent intent=new
				 * Intent(context, RegisterPage.class);
